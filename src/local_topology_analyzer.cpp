...
namespace ConverterLib
{
  ...
  LocalTopologyAnalyzer::LocalTopologyAnalyzer(const AABB &window, int Nx, int Ny) : window(window),
                                                                                     Nx(Nx),
                                                                                     Ny(Ny)
  {
    if (Nx <= 0 || Ny <= 0)
      ...
      for (int coord_A_y = 0; coord_A_y < Ny; coord_A_y++)
      {
        ...
        for (int coord_B_x = coord_A_x - 1; coord_B_x <= coord_A_x + 1; coord_B_x++)
          for (int coord_B_y = coord_A_y - 1; coord_B_y <= coord_A_y + 1; coord_B_y++)
          {
            ...
            if ((coord_B_x >= 0 && coord_B_x < Nx && coord_B_y >= 0 && coord_B_y < Ny) && (coord_A_x != coord_B_x || coord_A_y != coord_B_y))
            {
              ...
              double distance = sqrt(pow(coord_A_x - coord_B_x, 2.) + pow(coord_A_y - coord_B_y, 2.));
              if (distance == 0.)
              {
                // set to eps to avoid division by zero
                distance = 1e-10;
              }
          ...
          for (int ix = 0; ix < Nx; ix++)
            for (int iy = 0; iy < Ny; iy++)
            {
              ...
              for (int i = 0; i < 4; i++)
              {
                ...
                if (ix != Nx - 1)
                {
                  topo_indexes_gradient[ix][iy].grad[i].x = topo_indexes_avg[ix + 1][iy].ind[i] - topo_indexes_avg[ix][iy].ind[i];
                }
                else
                ...
                if (iy != Ny - 1)
                {
                  topo_indexes_gradient[ix][iy].grad[i].x = topo_indexes_avg[ix][iy + 1].ind[i] - topo_indexes_avg[ix][iy].ind[i];
                }
                else
                { // Gradient on the right boundary.
